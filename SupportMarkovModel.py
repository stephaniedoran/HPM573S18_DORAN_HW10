...
def print_comparative_outcomes(simOutputs_none, simOutputs_anticoag):
    ...
    # increase in survival time under combination therapy with respect to mono therapy
    change_num_strokes = Stat.DifferenceStatIndp(
        name='Difference in expected number of strokes',
        x=simOutputs_anticoag.get_if_developed_stroke(),
        y_ref=simOutputs_none.get_if_developed_stroke())

    # estimate and CI
    estimate_CI = F.format_estimate_interval(
        estimate=change_num_strokes.get_mean(),
        interval=change_num_strokes.get_t_CI(alpha=Settings.ALPHA),
        deci=2)
    print("Average difference in expected number of strokes "
          "and {:.{prec}%} confidence interval:".format(1 - Settings.ALPHA, prec=0),
          estimate_CI)
